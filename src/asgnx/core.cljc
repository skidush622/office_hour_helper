...
;; that takes the current application `state`, a `course`
;; the TA's `id` (e.g., unique name), and information
;; about the TA (`info`) and registers them as an TA on
;; the specified course. Look at the associated test to see the
;; expected function signature.
;;
;; NOT directly change the application state
...
;; that takes the current application `state`, a `course`
;; and the TA's `id` (e.g., unique name) and then
;; removes the TA from the list of TAs on that course.
...
;;
;; Parameters: user-id which is the phone number of sender
;;
;; This function is used to loop the vector and find the position of the sender
;; the number of how many elements before the sender is returned
;;
;; It is called in ask-experts function and query-student function
;;
...
;; Parameters:
;;
;; 1. the list of TAs on the course
...
;; [[actions...] "response to student"]
;;
;; Enqueue the student into the queue.
;;
;; The actions in the list are the *side effects* that need to take place
;; to ask the question.
;; The string is the response that is going to be sent back to the person that registered
;; the office hour.
;;
;; If there are no registered experts on a topic, you should return an
;; empty list of actions and "There are no TAs on that course."
;;
...
;;     :args []}
;;
;; The parsed message is generated by breaking up the words in the query
;; text message. For example, if someone sent the message:
;;
;; "query"
;;
...
;;  :args []}
;;
;; This function is used to return a String to tell your position in the queue
;; like "3 people before you."
;;
...
;;No parameter
;;
;;This function is used to deque the peek of the queue
;;and return a String
;;
;;This function is called in answer-question function
;;
(defn notify []
...
;;     :args []}
;;
;; The parsed message is generated by breaking up the words in the notify
;; text message. For example, if someone sent the message:
;;
;; "notify"
;;
...
;;
;; This function needs to return a list with two elements:
;; [[actions...] "response to TA notifying"]
;;
;; The actions in the list are the *side effects* that need to take place
;; to send the answer to the original question asker. The string
;; is the response that is going to be sent back to the TA
;;
;; Why this strange architecture? By returning a list of the actions to take,
;; rather than directly taking that action, we can keep this function pure.
;; Pure functions are WAY easier to test / maintain. Also, we can isolate our
...
(defn answer-question [{:keys [args user-id]}]
  (if (= 0 (count Queue))
    [[] "The Queue is Empty."]
...
;;parameters:
;; 1. the current list of TAs on the course
;; 2. a parsed message with the format:
;;    {:cmd "TA"
;;     :user-id "+15555555555"
;;     :args [course]
;;
;;
;; The parsed message is generated by breaking up the words in the TA
;; text message. For example, if someone sent the message:
;;
;; "TA math"
;;
...
;;
;; This function needs to return a list with two elements:
;; [[actions...] "response to the person adding themselves as an TA"]
;;
;; The actions in the list are the *side effects* that need to take place
;; to add the person as an TA on the topic (hint: result of calling experts-register). The string
...
(defn add-expert [experts {:keys [args user-id]}]
  [(experts-register experts (first args) user-id {:foo "bar"}) (str user-id " is now a teaching assistant on " (first args) ".")])
